...
export class GameScene extends Phaser.Scene {
  ...
  private spawnUnit(
    ...
    const hpMul = (boss ? 4.0 : 1) * modHp;  // heavy 250 -> boss 1000 HP. COIN-FLIP CONFIRMED — HOLD this config (boss 1000 HP AND W15 count=12). THREE held-config samples now bracket the boundary on a genuine knife's edge: run 85 HEAL entry 750 -> WON 270 (-480 dmg); run 86 NO-HEAL entry 500 -> LOST (min 20, near-miss wipe); run 87 HEAL entry 750 -> LOST -90 (~840 dmg). The HEADLINE of run 87: a HEAL run (entry 750, natural draw) still LOST — the W15 climax damage is HIGH-VARIANCE (480 vs 840 at IDENTICAL config/entry), so the outcome is RNG-decided (transit-kill variance × heal draw), the design goal. Both losses are within ~1 leaked boss of surviving = no wide-margin wipe. 1W/2L = 33% over n=3 is within binomial noise of 50% BUT the point estimate sits just below the 40-60 band, AND the 360-dmg spread at identical HP proves a +-5% boss-HP nudge is below the noise floor (do NOT nudge — it would be tuning on noise). W1-W14 take ZERO damage every run (pre-boss trivial). DO NOT crank boss HP and DO NOT touch W15 count — the lever has landed. NEXT run (88): HOLD a 4th time, take one more natural sample to reach n=4. If 88 wins -> 2W/2L=50% -> set win_rate_40_60=true and SETTLE. If 88 loses by a CLOSE margin -> 1W/3L=25% (genuinely below band) -> -10% boss HP (->900) to recenter, n resets. If any sample hard-wipes by a wide margin (dead with ai>=4 leaking) -> -10%. Secondary future pivot (after the flag is locked): give W1-W14 teeth, since avg_duration ~430s is 2.4x over the 180s target.
