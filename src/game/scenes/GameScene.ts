...
export class GameScene extends Phaser.Scene {
  ...
  private spawnUnit(
    ...
    const hpMul = (boss ? 4.0 : 1) * modHp;  // heavy 250 -> boss 1000 HP. COIN-FLIP CONFIRMED at this config (HOLD it — boss 1000 HP AND W15 count=12). Two mixed samples at the identical held config now bracket the win/loss boundary exactly as run 84 predicted: run 85 HEAL entry 750 -> WON 270 (-480 dmg); run 86 NO-HEAL entry 500 -> LOST (W15 took ~480, hit min 20, then tipped to wipe). The W15 boss reliably deals ~480-500 dmg, so the climax outcome is decided purely by whether the mound heal lifts entry 500->750 -> a real ~50% coin-flip driven by an in-game draw, which is the design goal. W1-W14 take ZERO damage every run (pre-boss trivial). DO NOT crank boss HP further and DO NOT touch W15 count — the lever has landed. Next run: HOLD again for a 3rd (natural-draw) sample, then mark win_rate_40_60 and settle. Only if 3+ samples drift >60% win -> +5% boss HP; if a clean no-heal entry hard-wipes by a wide margin (dead with ai>=4 leaking) -> -10%. Secondary future pivot (after the flag is locked): give W1-W14 teeth, since avg_duration ~414s is 2.3x over the 180s target.
