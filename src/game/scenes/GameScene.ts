...
export class GameScene extends Phaser.Scene {
  ...
  private spawnUnit(
    ...
    const hpMul = (boss ? 3.6 : 1) * modHp;  // heavy 250 -> boss 900 HP. RECENTER from 1000 (-10%) per run 87's pre-committed rule. FOUR held-config samples at boss 1000 HP / count=12 read 1W/3L = 25%, below the 40-60 band: run 85 HEAL entry 750 -> WON 270 (-480 dmg); run 86 NO-HEAL entry 500 -> LOST (min 20); run 87 HEAL entry 750 -> LOST -90 (-840 dmg); run 88 HEAL entry 690 -> LOST -30 (-720 dmg). W15 climax damage is HIGH-VARIANCE (480-840 across identical-config samples) -> outcome is RNG-decided (transit-kill variance x heal draw) = the design goal, BUT the rate sits too loss-heavy (75% loss). This -10% step (1000->900) is decisively ABOVE the noise floor (a 100-HP drop, unlike the +-5% nudge run 87 correctly rejected as noise-tuning): ~1.6 fewer stinger shots/boss -> a touch more transit-kill, fewer leakers -> shifts the whole W15 damage distribution down to recenter wins toward 50%. Did NOT touch W15 count (integer lever too coarse: 12=this, 13=hard wipe) and did NOT touch the pre-boss (W1-W14 trivially zero-damage every run). Run 89 (FIRST sample at boss 900, NATURAL no-heal draw): CLOSE LOSS at W15 (hp -40, enemiesLeft=0 = closest possible loss, all bosses dead, base overran by 40). KEY DATA: W15 @900 dealt ~360 dmg (entry 320 -> final -40), MARKEDLY lower than the ~480 it dealt @1000 -> the -10% recenter shifted the climax distribution DOWN as intended. BUT entry was only 320 (not the clean 500): the pre-boss W9 (wi 8, 6 medium all-lane) leaked 180 HP this run -> a high-variance pre-boss spike contaminated the W15 entry. Had entry been the clean 500, 500-360 = +140 margin -> WOULD HAVE WON. So this loss is attributable to pre-boss-leak variance (W9 -180), NOT a too-hard W15 climax. Per run 88's pre-committed rule for this exact branch ('if 89 loses CLOSE -> sample more before judging'), HOLD config (no balance change; only this doc comment + state updated, gameplay byte-identical). n=1 at boss 900 is too few to judge, and the loss was pre-boss-contaminated, not a clean-entry climax loss -> changing anything now would repeat the loop's documented anti-pattern of never holding still. NEXT run (90): HOLD config (boss 900 / count 12), sample again. If a CLEAN-entry (pre-boss zero-leak) run at 900 wins comfortably -> the climax is well-centered and the real decider is pre-boss W9 leak variance (0-180 HP observed across runs 85-89) -> pivot to STABILIZING pre-boss W9 leak (throughput, per #48/#51), NOT touching boss HP. If clean-entry 900 also loses -> climax still too hard, nudge boss HP down further. If 2-3 more samples read >=2 wins -> coin-flip recentered, lock win_rate_40_60.
